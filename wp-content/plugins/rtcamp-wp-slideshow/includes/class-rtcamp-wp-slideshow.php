<?php
/**
 * Class file for Rtcamp_Wp_Slideshow.
 *
 * @package    Rtcamp_Wp_Slideshow
 */

/**
 * Class for creating and managing the slideshow.
 *
 * @package    Rtcamp_Wp_Slideshow
 */
class Rtcamp_Wp_Slideshow {
	...
	public function theme_enqueue_scripts( $hook ) {
		...
		wp_enqueue_style( 'swiper-css', 'https://unpkg.com/swiper/swiper-bundle.min.css', array(), '6.5.8' );
	...
	public function admin_page() {
		...
		$action = isset( $_GET['action'] ) ? sanitize_text_field( wp_unslash( $_GET['action'] ) ) : '';
		if ( 'delete' === $action ) {
			check_admin_referer( 'rtcamp-wp-slideshow-delete-slider' );
			$this->delete_slider();
			return;
		}
		...
		<div class="wrap">
			...
			?>
						" class="button button-primary"><?php esc_html_e( 'Add New Slider', 'rtcamp-wp-slideshow' ); ?></a>
		</div>
		<?php
	}
	...
	public function add_slider_page() {
		if ( isset( $_POST['submit'] ) ) {
			// Verify the nonce.
			if ( ! isset( $_POST['add_slider_nonce'] ) || ! wp_verify_nonce( wp_unslash( $_POST['add_slider_nonce'] ), 'add_slider' ) ) {
				return;
			}
			$this->save_slider();
			return;
	...
	public function save_slider() {
		...
		$slider_status = isset( $_POST['status'] ) ? sanitize_text_field( wp_unslash( $_POST['status'] ) ) : '';
		$slider_images = isset( $_POST['slider_images'] ) ? array_map( 'sanitize_text_field', wp_unslash( $_POST['slider_images'] ) ) : array();

		// Serialize the slider images.
		$slider_images = serialize( $slider_images );

		// Get the slider order from the hidden input field.
		$slider_order = isset( $_POST['slider_order'] ) ? sanitize_text_field( wp_unslash( $_POST['slider_order'] ) ) : '';
		...
		echo '<script>window.location.href="' . esc_url( admin_url( 'admin.php?page=rtcamp-wp-slideshow&message=slider_added' ) ) . '";</script>';
		exit;
	}
	...
	public function edit_slider_page() {
		...
		$slider = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM $table_name WHERE id = %d", $slider_id ) );//phpcs:ignore WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.SchemaChange

		echo '<div class="wrap">';
		echo '<h1>' . esc_html( get_admin_page_title() ) . '- Edit Slider: ' . esc_attr( $slider_id ) . ' <button type="button" id="add-slide" class="button">Add Slide</button></h1>';
		...
		if ( isset( $_POST['submit'] ) && check_admin_referer( 'edit_slider_' . $slider_id ) ) {
			...
			$slider_name   = isset( $_POST['name'] ) ? sanitize_text_field( wp_unslash( $_POST['name'] ) ) : '';
			$slider_status = isset( $_POST['status'] ) ? sanitize_text_field( wp_unslash( $_POST['status'] ) ) : '';
		...
		if ( is_array( $slider_images ) && count( $slider_images ) > 0 ) {
			...
			foreach ( $slider_images as $key => $image ) {
				...
				echo '<div class="slide" slider-id="' . esc_attr( $slide_id ) . '">';
				echo '<h4>Slide ' . esc_html( $slide_number ) . '</h4>';
				echo '<img src="' . esc_attr( $image ) . '" height="96" width="96" loading="lazy" />';
				echo '<input type="hidden" name="slider_images[' . esc_attr( $key ) . ']" value="' . esc_attr( $image ) . '" />';
	...
	public function delete_slider() {
		...
		$slider = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$table_name} WHERE id = %d", $slider_id ), ARRAY_A );//phpcs:ignore WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.SchemaChange
		...
		if ( isset( $_POST['submit'] ) && check_admin_referer( 'delete_slider_' . $slider_id ) ) {
			// Delete the slider.
			$delete_result = wp_cache_delete( $table_name, array( 'id' => $slider_id ) );

			if ( false === $delete_result ) {
				printf( '<div class="notice notice-error is-dismissible"><p>%s</p></div>', esc_html__( 'Slider delete failed.', 'rtcamp-wp-slideshow' ) );
			} else {
				// Use JavaScript to redirect instead of wp_redirect().
				echo '<script>window.location.href="' . esc_url( admin_url( 'admin.php?page=rtcamp-wp-slideshow&message=slider_deleted' ) ) . '";</script>';
				exit;
			}
		}

		echo '<div class="wrap">';
		echo '<h1>' . esc_html( get_admin_page_title() ) . '- Delete Slider: ' . esc_attr( $slider_id ) . '</h1>';
	...
	public function rtcamp_wp_slideshow_shortcode( $atts ) {
		...
		// Cache the slider data using the slider ID as the cache key.
		$slider_data = wp_cache_get( $slider_id, 'slider_data' );

		if ( false === $slider_data ) {
			// If the slider data is not in the cache, query the database and store the result in the cache.
			global $wpdb;
			$table_name  = $wpdb->prefix . 'rtcamp_wp_slideshow';
			$slider_data = $wpdb->get_row( //phpcs:ignore WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.SchemaChange
				$wpdb->prepare( "SELECT * FROM {$table_name} WHERE id = %d", $slider_id ),
				ARRAY_A
			);
			wp_cache_set( $slider_id, $slider_data, 'slider_data' );
		}
		...
		if ( 'published' !== $slider_data['status'] ) {
